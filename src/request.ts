...
export default async (input: string): Promise<string | null> =>{
	...
	try{
	promise = fetch(vscode.workspace.getConfiguration("nearcoder").get("apiurl") as string,
		{
			// eslint-disable-next-line @typescript-eslint/naming-convention
			headers: { authorization: `Bearer ${vscode.workspace.getConfiguration("nearcoder").get("bearertoken")}`, "content-type": "application/json" },
			method: "POST",
	...
	let output = ((await response.json() as Promise<ResponseModel[]>))[0].generated_text;
